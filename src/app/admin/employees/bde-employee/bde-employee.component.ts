...
export class BdeEmployeeComponent extends UnsubscribeOnDestroyAdapter
  ...
  ngOnInit() {
    ...
    this.exampleDatabase.getAllEmployeess().subscribe({
      next: (data) => {
        const filteredData = data.filter(employee => employee.role === 'BDE');
        this.exampleDatabase!.dataChange.next(filteredData);
        this.loadData(); // Load after data is ready
      },
      error: (error) => {
  ...
  connect(): Observable<Employees[]> {
    ...
    this.exampleDatabase.getAllEmployeess().subscribe({
      next: (data) => {
        ...
        // Filter the data to show only employees with the role 'BDE'
        const filteredData = data.filter(employee => employee.role === 'BDE');
        this.exampleDatabase!.isTblLoading = false;
        this.exampleDatabase!.dataChange.next(filteredData); // Use filtered data
      },
      error: (error) => {
        this.exampleDatabase!.isTblLoading = false;
