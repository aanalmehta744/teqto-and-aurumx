...
import { PayrollService } from '../payslip/payroll.service';
import { AddPayslipComponent } from './dialogs/add-payslip/add-payslip.component';
...
export class EmployeeSalaryComponent
  ...
  downloadCall(row: EmployeeSalary) {
    const dialogRef = this.dialog.open(FormDialogComponent, {
      width: '300px',
      data: row
    });
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        // this.downloadPayslip(row.id, month, year);
      }
    });
  }
  openPayslipForm(row: EmployeeSalary) {
    const dialogRef = this.dialog.open(AddPayslipComponent, {
      width: '500px',
      data: row
    });
      ...
      map(() => {
        ...
        this.filteredData = this.exampleDatabase.data
          ...
          .filter((employeeSalary: EmployeeSalary) => {
            ...
            const loggedDept = loggedUser?.department?.toLowerCase();
            const loggedEmpId = loggedUser?.id; // or employee_id

            // 👮 Department rule
            if (loggedDept !== 'hr') {
              // Non-HR → show only own record
              if (employeeSalary.id !== loggedEmpId) {
                return false;
              }
            }
