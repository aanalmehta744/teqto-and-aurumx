...
(async () => {
  try {
    ...
    // Column already exists on repeat boot - safe to ignore
  }
})();
...
if (!q || !employeeId) {
  return res.status(400).json({
    error: "q and employeeId are required"
  });
}
...
if (!sender_id || !receiver_id || sender_id === receiver_id) {
  return res.status(400).json({
    error: "Valid sender_id and receiver_id are required"
  });
}
...
if (existingReq.length > 0) {
  ...
  io.to(`user_${receiver_id}`).emit("chat_request_received", {
    id: existing.id,
    sender_id,
    receiver_id
  });
...
try {
  ...
  await connection.query(
    "UPDATE chat_requests SET status = 'accepted', conversation_id = ?, responded_at = NOW() WHERE id = ?",
  ...
  io.to(`user_${request.sender_id}`).emit("chat_request_accepted", {
    request_id: requestId,
    conversationId,
    by_employee_id: request.receiver_id
  });
...
https.get(attachment_url, (fileRes) => {
  ...
  if (attachment_type) {
    res.setHeader("Content-Type", attachment_type);
  }
