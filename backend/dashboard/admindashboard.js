...
// GET: KPI target achievement breakdown per BDE
router.get('/bde-kpi-achievement', async (req, res) => {
  ...
  try {
    ...
    const results = await Promise.all(bdes.map(async (bde) => {
      ...
      return { bde_id: bde.id, bde_name: bde.fullName, total_kpis: targets.length, completed, partial, missed, avg_achievement: avgPct };
    }));
...
// GET: BDE performance summary for admin dashboard graph
router.get('/bde-performance-summary', async (req, res) => {
  ...
  try {
    ...
    const results = await Promise.all(bdes.map(async (bde) => {
      ...
      return { bde_id: bde.id, bde_name: bde.fullName, calls: calls.cnt, meetings: meetings.cnt, deals_won: deals.cnt, revenue: revenue.total };
    }));
