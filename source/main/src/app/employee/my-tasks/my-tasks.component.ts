...
export class MyTasksComponent
  ...
  groupedTasks: { date: string; tasks: MyTasks[] }[] = [];
  ...
  private groupByDate(tasks: MyTasks[]): void {
    const groups: { [key: string]: MyTasks[] } = {};

    tasks.forEach((task) => {
      const key = formatDate(task.due_date, 'yyyy-MM-dd', 'en'); // group by due_date
      if (!groups[key]) {
        groups[key] = [];
      }
    ...
    // convert object into array sorted by date (desc)
    this.groupedTasks = Object.keys(groups)
