      ...
      map(() => {
        ...
        this.filteredData = this.exampleDatabase.data
          ...
          .filter((leaves: Leaves) => {
            // Convert date fields to multiple searchable formats
            const fromDate = leaves.end_date
              ? [
                new Date(leaves.end_date).toLocaleDateString('en-US', { day: '2-digit', month: 'short', year: 'numeric' }), // "Aug 14, 2025"
                new Date(leaves.end_date).toLocaleDateString('en-GB'), // "14/08/2025"
                formatDate(new Date(leaves.end_date), 'yyyy-MM-dd', 'en'), // "2025-08-14"
                formatDate(new Date(leaves.end_date), 'dd MMM yyyy', 'en') // "14 Aug 2025"
              ].join(' ')
              : '';

            const toDate = leaves.start_date
              ? [
                new Date(leaves.start_date).toLocaleDateString('en-US', { day: '2-digit', month: 'short', year: 'numeric' }),
                new Date(leaves.start_date).toLocaleDateString('en-GB'),
                formatDate(new Date(leaves.start_date), 'yyyy-MM-dd', 'en'),
                formatDate(new Date(leaves.start_date), 'dd MMM yyyy', 'en')
              ].join(' ')
              : '';
